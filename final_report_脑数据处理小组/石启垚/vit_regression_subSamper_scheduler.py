...
def test_model(test_dataset,batch_size, model, device, 
               rating_cols, sd_cols, criterion, batches_to_test=16):
    model.eval()
    total_test_loss = 0.0
    total_error = 0.0
    total_confidence = 0.0  # 用于累加每个 batch 的平均置信度
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            sds = torch.stack(
                [batch[col].to(device).float() for col in sd_cols],
                dim=1
                )
            ...
            # 计算置信度
            z_scores = (outputs - targets) / sds.clamp(min=1e-6)
            p_values = 2 * normal.cdf(-z_scores.abs())
            batch_cof = 1 - p_values.mean().item()
            total_confidence += batch_cof

            if batch_idx % batches_to_test == 0:
                tqdm.write(f"Test Batch {batch_idx}/{batches_to_test} | "
                           f"Loss: {loss.item():.4f}|"
                           f"Batch Conf: {batch_cof:.4f}")
    avg_test_loss = total_test_loss / batches_to_test
    avg_test_error = total_error / batches_to_test
    avg_confidence = total_confidence / batches_to_test
    # 打印测试结果
    print(f"\nTest Loss: {avg_test_loss:.4f} | "
          f"Test Avg Error: {avg_test_error:.4f}|"
          f"Test Avg Confidence: {avg_confidence:.4f}\n")
...
if __name__ == "__main__":
    ...
    if Test:
        model.load_state_dict(torch.load("./results/model/vit3d.pth",weights_only=True))
        print("Testing model...")
        test_model(test_dataset,config["batch_size"], model, device, 
                rating_cols, SD_cols, criterion, config["batches_to_test"])
