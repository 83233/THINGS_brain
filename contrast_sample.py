...
class PairDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {'image_i': img_i, 'image_j': img_j,
                ...
                'sd_j'      : prop_sd_j,     # [10,]
                'idx_i'     : i,             # 正对中第一个样本在 base_ds 中的原始索引
                'idx_j'     : j              # 正对中第二个样本在 base_ds 中的原始索引
            }
...
def train(model, full_train_ds, optimizer, device,
          ...
          lambda_prop: float = 0.1, random_seed: int = 42,
...
def create_datasets(
    ...
    """创建基础数据集和正样本对数据集"""
    base_ds = THINGSImageDataset(meta_path, img_root, category27_tsv_path=None, transform=transform)
...
if __name__ == '__main__':
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        ...
        patience=2,            # 若 2 个 epoch 内 loss 无下降，则触发降 LR
